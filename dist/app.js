...
document.addEventListener('click',e=>{
  ...
  if(action==='create-document-order'){ const index=Number(target.dataset.document),doc=state.documents[index]; if(doc&&doc.status==='matched'){ doc.status='created';doc.progress=4;doc.orderId='BER-260910-258';doc.erpId='ABACUS #98456';if(!state.adminOrders.some(o=>o.id===doc.orderId))state.adminOrders.unshift({id:doc.orderId,partner:doc.customer,initials:initials(doc.customer),date:'10. sep 2026.',items:doc.lines,total:doc.total,status:'processing',label:'Poslato u Abacus',progress:2});renderShell();showDocument(index);toast(state.lang==='en'?'Order created and recorded in Abacus.':'Porudžbina je kreirana i evidentirana u Abacusu.');} }
