...
function adminIntegration() {
  const mappings=[
    ['Artikli i lager','Products & stock','getItems',state.lang==='en'?'Code, name, quantity, price, VAT and unit':'Šifra, naziv, količina, cijena, PDV i jedinica'],
    ['Cijene i rabati kupca','Customer pricing & discounts','getItems + id_partner / id_c',state.lang==='en'?'Calculated by Abacus for the selected customer':'Abacus obračunava za izabranog kupca'],
    ['Poslovni kupci','Business customers','getPartners',state.lang==='en'?'Partner identity and contact data':'Identitet i kontakt podaci partnera'],
    ['Radne jedinice','Work units','getWorkUnits · id_m 4',state.lang==='en'?'Wholesale warehouse selected for the sample':'Veleprodajni magacin izabran za primjer'],
    ['Prodajne porudžbine','Sales orders','postSale',state.lang==='en'?'Portal order is created in Abacus':'Porudžbina sa portala kreira se u Abacusu'],
    ['Finansijsko stanje','Financial status','getPartnerBalanceDetailed / getOpenItems',state.lang==='en'?'Available for credit and open-item checks':'Dostupno za provjeru salda i otvorenih stavki'],
  ];
  const questions=[
    [state.lang==='en'?'Product content':'Sadržaj proizvoda',state.lang==='en'?'Images and descriptions are empty in the supplied getItems sample. Test getImages or agree another content source.':'Slike i opisi su prazni u dostavljenom getItems primjeru. Testirati getImages ili dogovoriti drugi izvor sadržaja.'],
    [state.lang==='en'?'Order reference and status':'Referenca i status porudžbine',state.lang==='en'?'The documentation confirms HTTP 200/500, but not the returned Abacus document ID or status feed.':'Dokumentacija potvrđuje HTTP 200/500, ali ne i povratni ID Abacus dokumenta niti tok statusa.'],
    [state.lang==='en'?'Production access':'Produkcioni pristup',state.lang==='en'?'Confirm HTTPS, production credentials, rate limits, pagination, sync cadence and Bencom API access pricing.':'Potvrditi HTTPS, produkcione pristupne podatke, limite, paginaciju, ritam sinhronizacije i Bencom cijenu API pristupa.'],
  ];
  return `<main class="page">${pageHeader('Bencom API · protocol 2.0',state.lang==='en'?'Abacus API connection':'Abacus API povezivanje',state.lang==='en'?'The integration approach is now validated against the supplied Bencom documentation, Postman collection and Berba test response.':'Način povezivanja je sada provjeren prema dostavljenoj Bencom dokumentaciji, Postman kolekciji i Berba testnom odgovoru.',`<button class="secondary-button" data-action="integration-details">${icon('file')} ${state.lang==='en'?'View API mapping':'Pogledaj API mapiranje'}</button>`)}<div class="api-confirmed-banner"><span class="api-confirmed-icon">${icon('check')}</span><div><strong>${state.lang==='en'?'Technical feasibility confirmed':'Tehnička izvodljivost potvrđena'}</strong><span>${state.lang==='en'?'Bencom confirmed products, partners, price lists, stock and order creation through the Abacus API. Access credentials remain protected server-side.':'Bencom je potvrdio artikle, partnere, cjenovnike, lager i kreiranje porudžbine kroz Abacus API. Pristupni podaci ostaju zaštićeni na serverskoj strani.'}</span></div><span class="status active">${state.lang==='en'?'API confirmed':'API potvrđen'}</span></div><div class="api-summary-grid"><div class="api-summary-card"><span class="api-summary-icon">${icon('box')}</span><div><code>getItems</code><strong>${state.lang==='en'?'8,585 records parsed':'8.585 zapisa obrađeno'}</strong><span>${state.lang==='en'?'Berba test response · warehouse 4':'Berba testni odgovor · magacin 4'}</span></div></div><div class="api-summary-card"><span class="api-summary-icon">${icon('tag')}</span><div><code>id_partner + id_c</code><strong>${state.lang==='en'?'Customer-calculated price':'Cijena obračunata za kupca'}</strong><span>${state.lang==='en'?'No recalculation needed in B2Bware':'Bez ponovnog obračuna u B2Bware-u'}</span></div></div><div class="api-summary-card"><span class="api-summary-icon">${icon('cart')}</span><div><code>postSale</code><strong>${state.lang==='en'?'Order write-back confirmed':'Upis porudžbine potvrđen'}</strong><span>${state.lang==='en'?'Partner, items, quantities and note':'Partner, stavke, količine i napomena'}</span></div></div><div class="api-summary-card"><span class="api-summary-icon">${icon('building')}</span><div><code>id_m: 4</code><strong>${state.lang==='en'?'Wholesale work unit':'Radna jedinica veleprodaje'}</strong><span>${state.lang==='en'?'Stock is requested per Abacus work unit':'Lager se traži po Abacus radnoj jedinici'}</span></div></div></div><section class="panel"><div class="panel-head"><div><div class="panel-title">${state.lang==='en'?'Confirmed order flow':'Potvrđen tok porudžbine'}</div><div class="panel-subtitle">${state.lang==='en'?'The portal creates a sales order; Berba later realises it and creates the invoice in Abacus.':'Portal kreira prodajnu porudžbinu; Berba je zatim realizuje i kreira račun u Abacusu.'}</div></div><span class="endpoint-pill">POST · postSale</span></div><div class="panel-body"><div class="integration-flow"><div class="flow-node"><div><span class="flow-icon">${icon('wine')}</span><div class="flow-title">${state.lang==='en'?'Business customer':'Poslovni kupac'}</div><div class="flow-copy">${state.lang==='en'?'Customer catalogue, Abacus-calculated pricing, stock and cart':'Katalog kupca, cijena iz Abacusa, lager i korpa'}</div></div></div><span class="flow-arrow">${icon('arrow')}</span><div class="flow-node primary"><div><span class="flow-icon">${icon('sync')}</span><div class="flow-title">B2Bware</div><div class="flow-copy">${state.lang==='en'?'Validates partner and lines, then sends the postSale payload':'Validira partnera i stavke, zatim šalje postSale zahtjev'}</div></div></div><span class="flow-arrow">${icon('arrow')}</span><div class="flow-node"><div><span class="flow-icon">${icon('building')}</span><div class="flow-title">Abacus ERP</div><div class="flow-copy">${state.lang==='en'?'Creates the order; realisation, invoice and fiscal process stay in Abacus':'Kreira porudžbinu; realizacija, račun i fiskalni proces ostaju u Abacusu'}</div></div></div></div></div></section><div class="integration-review-grid"><section class="panel"><div class="panel-head"><div><div class="panel-title">${state.lang==='en'?'Confirmed API mapping':'Potvrđeno API mapiranje'}</div><div class="panel-subtitle">${state.lang==='en'?'Endpoints verified in the supplied documentation':'Endpointi provjereni u dostavljenoj dokumentaciji'}</div></div></div><div class="api-map-list">${mappings.map(([me,en,endpoint,copy])=>`<div class="api-map-row"><div><strong>${state.lang==='en'?en:me}</strong><span>${copy}</span></div><code>${endpoint}</code><span class="status active">${state.lang==='en'?'Confirmed':'Potvrđeno'}</span></div>`).join('')}</div></section><section class="panel"><div class="panel-head"><div><div class="panel-title">${state.lang==='en'?'Confirm during setup':'Potvrditi tokom setup-a'}</div><div class="panel-subtitle">${state.lang==='en'?'Specific implementation details, not blockers':'Detalji implementacije, ne blokiraju projekat'}</div></div></div><div class="api-question-list">${questions.map(([title,copy])=>`<div class="api-question"><span class="api-question-icon">${icon('info')}</span><div><strong>${title}</strong><span>${copy}</span></div></div>`).join('')}</div></section></div></main>`;
}
...
function showIntegrationDetails() {
  const itemsPayload=`{
  "id_m": 4,
  "id_partner": "<Abacus partner ID>",
  "id_c": "<price list ID>"
}`;
  const orderPayload=`{
  "order": {
    "id_partner": "<Abacus partner ID>",
    "note": "Portal order reference",
    "items": [
      { "id_item": "<Abacus item ID>", "quantity": 4 }
    ]
  },
  "id_m": 4
}`;
  modalRoot.innerHTML=`<div class="modal-overlay" data-action="close-modal"><article class="modal wide" role="dialog" aria-modal="true" aria-label="${state.lang==='en'?'Abacus API mapping':'Abacus API mapiranje'}" onclick="event.stopPropagation()"><div class="modal-head"><div><div class="modal-title">${state.lang==='en'?'Validated Abacus API mapping':'Provjereno Abacus API mapiranje'}</div><div class="panel-subtitle">Bencom · protocol 2.0 · ${state.lang==='en'?'credentials hidden':'pristupni podaci sakriveni'}</div></div><button class="icon-button" data-action="close-modal" aria-label="${state.lang==='en'?'Close':'Zatvori'}">${icon('close')}</button></div><div class="modal-body"><div class="api-modal-intro"><span class="api-confirmed-icon">${icon('check')}</span><div><strong>${state.lang==='en'?'The core Berba flow is supported':'Osnovni Berba tok je podržan'}</strong><span>${state.lang==='en'?'The supplied sample returned 8,585 item records with stock, price, discount, VAT, units and promotion fields.':'Dostavljeni primjer vratio je 8.585 zapisa artikala sa lagerom, cijenom, rabatom, PDV-om, jedinicom mjere i poljem promocije.'}</span></div></div><div class="api-payload-grid"><section><div class="panel-title">1. getItems</div><div class="panel-subtitle">${state.lang==='en'?'Products, warehouse stock and customer pricing':'Artikli, stanje magacina i cijene kupca'}</div><div class="api-method-line"><span class="api-method get">GET</span><code>/Berba/Main/getItems?protocol=2.0</code></div><pre class="api-payload"><code>${esc(itemsPayload)}</code></pre></section><section><div class="panel-title">2. postSale</div><div class="panel-subtitle">${state.lang==='en'?'Create the sales order in Abacus':'Kreiranje prodajne porudžbine u Abacusu'}</div><div class="api-method-line"><span class="api-method post">POST</span><code>/Berba/Main/postSale?protocol=2.0</code></div><pre class="api-payload"><code>${esc(orderPayload)}</code></pre></section></div><div class="api-modal-notes"><div class="api-note good">${icon('check')}<span><strong>${state.lang==='en'?'Confirmed':'Potvrđeno'}:</strong> ${state.lang==='en'?'partners, work units, price lists, items, stock, calculated prices/discounts and order creation.':'partneri, radne jedinice, cjenovnici, artikli, lager, obračunate cijene/rabati i kreiranje porudžbine.'}</span></div><div class="api-note review">${icon('info')}<span><strong>${state.lang==='en'?'To confirm':'Potvrditi'}:</strong> ${state.lang==='en'?'HTTPS and production access, sync limits, image source, returned order reference and status/document mapping.':'HTTPS i produkcioni pristup, limite sinhronizacije, izvor slika, povratnu referencu porudžbine i mapiranje statusa/dokumenata.'}</span></div></div></div><div class="modal-foot"><button class="primary-button" data-action="close-modal">${state.lang==='en'?'Done':'U redu'}</button></div></article></div>`;
...
function showOrder(id) {
  ...
  const erpMeta=o.erpId||(o.progress>=2?'postSale · HTTP 200':(state.lang==='en'?'Awaiting approval and postSale':'Čeka odobrenje i postSale'));
...
document.addEventListener('click',e=>{
  ...
  if(action==='approve-order'){ const o=findOrder(target.dataset.order); if(o){o.status='processing';o.label='Poslato u Abacus';o.progress=2;o.erpId='postSale · HTTP 200';closeModal();renderShell();toast(state.lang==='en'?'Order approved and created through postSale.':'Porudžbina je odobrena i kreirana kroz postSale.');} }
