...
const state = {
  ...
  partnerOrders:[...partnerOrders], adminOrders:[...adminOrders], uploadResult:null,
};
...
function adminDocuments() {
  const inputs=[['PO-REGENT-1009.pdf','Regent Porto Montenegro','9 stavki','Upareno','active'],['Narudzba-Casa-del-Mare.xlsx','Casa del Mare','6 stavki','Provjera kupca','review'],['order_voli_1009.csv','Voli Trade','17 stavki','Poslato u Abacus','processing'],['photo-order-8801.jpg','Restoran Galion','4 stavke','Kreirana porudžbina','active']];
  return `<main class="page">${pageHeader(state.lang==='en'?'Order automation':'Automatizacija naručivanja',state.lang==='en'?'Incoming documents':'Ulazni dokumenti',state.lang==='en'?'Purchase orders received by upload or email, matched to products and customers.':'Porudžbenice primljene kroz portal ili email, uparene sa proizvodima i kupcima.')}<section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>${state.lang==='en'?'Document':'Dokument'}</th><th>${state.lang==='en'?'Customer':'Kupac'}</th><th>${state.lang==='en'?'Recognised':'Prepoznato'}</th><th>Status</th><th>${state.lang==='en'?'Received':'Primljeno'}</th></tr></thead><tbody>${inputs.map((x,i)=>`<tr class="clickable-row" data-action="toast"><td><div class="mini-product"><span class="document-icon">${icon('file')}</span><strong>${x[0]}</strong></div></td><td>${x[1]}</td><td>${x[2]}</td><td><span class="status ${x[4]}">${state.lang==='en'?({Upareno:'Matched','Provjera kupca':'Customer review','Poslato u Abacus':'Sent to Abacus','Kreirana porudžbina':'Order created'})[x[3]]:x[3]}</span></td><td>${[2,11,24,42][i]} min</td></tr>`).join('')}</tbody></table></div></section></main>`;
}

function adminIntegration() {
  const entities=[['Proizvodi','Abacus → B2Bware','active'],['Stanje zaliha','Abacus → B2Bware','active'],['Kupci i status','Abacus → B2Bware','active'],['Cijene i rabati','Abacus → B2Bware','pending'],['Porudžbine','B2Bware → Abacus','pending']];
  return `<main class="page">${pageHeader(state.lang==='en'?'System connection':'Povezivanje sistema',t('integration'),state.lang==='en'?'Proposed two-way exchange between the customer portal and the existing Bencom Abacus system.':'Predložena dvosmjerna razmjena između portala za kupce i postojećeg Bencom Abacus sistema.',`<button class="secondary-button" data-action="toast">${icon('sync')} ${state.lang==='en'?'Run test sync':'Pokreni test'}</button>`)}<div class="assumption-banner"><strong>${state.lang==='en'?'Technical discovery note:':'Napomena za tehničku provjeru:'}</strong> ${state.lang==='en'?'The entities below represent the intended business flow. Exact API endpoints, field mappings and regulatory requirements will be confirmed from Bencom’s Abacus API documentation.':'Entiteti ispod predstavljaju željeni poslovni tok. Tačni API endpointi, mapiranje polja i regulatorni zahtjevi biće potvrđeni na osnovu Bencom Abacus API dokumentacije.'}</div><section class="panel"><div class="panel-head"><div><div class="panel-title">${state.lang==='en'?'Proposed order flow':'Predloženi tok porudžbine'}</div><div class="panel-subtitle">${state.lang==='en'?'Customer self-service with controlled ERP write-back':'Samostalno poručivanje kupca uz kontrolisani upis u ERP'}</div></div><span class="status active">Abacus · Bencom</span></div><div class="panel-body"><div class="integration-flow"><div class="flow-node"><div><span class="flow-icon">${icon('wine')}</span><div class="flow-title">${state.lang==='en'?'Business customer':'Poslovni kupac'}</div><div class="flow-copy">${state.lang==='en'?'Catalogue, customer prices, stock and order':'Katalog, ugovorene cijene, stanje i porudžbina'}</div></div></div><span class="flow-arrow">${icon('arrow')}</span><div class="flow-node primary"><div><span class="flow-icon">${icon('sync')}</span><div class="flow-title">B2Bware</div><div class="flow-copy">${state.lang==='en'?'Validation, approval rules and status tracking':'Validacija, pravila odobrenja i praćenje statusa'}</div></div></div><span class="flow-arrow">${icon('arrow')}</span><div class="flow-node"><div><span class="flow-icon">${icon('building')}</span><div class="flow-title">Abacus ERP</div><div class="flow-copy">${state.lang==='en'?'Products, inventory, customers, pricing and sales orders':'Proizvodi, zalihe, kupci, cijene i prodajne porudžbine'}</div></div></div></div></div></section><section class="panel"><div class="panel-head"><div><div class="panel-title">${state.lang==='en'?'Data entities for confirmation':'Entiteti za potvrdu'}</div><div class="panel-subtitle">${state.lang==='en'?'Subject to Bencom API documentation':'Zavisi od Bencom API dokumentacije'}</div></div></div><div class="panel-body"><div class="entity-grid">${entities.map(([name,dir,status])=>`<div class="entity-card"><div class="entity-head"><span class="entity-name">${state.lang==='en'?translateEntity(name):name}</span><span class="status ${status}">${status==='active'?(state.lang==='en'?'Expected':'Očekivano'):(state.lang==='en'?'To confirm':'Potvrditi')}</span></div><div class="entity-direction">${dir}<br>${state.lang==='en'?'Incremental synchronisation':'Inkrementalna sinhronizacija'}</div></div>`).join('')}</div></div></section></main>`;
}
...
document.addEventListener('click',e=>{
  ...
  if(action==='approve-order'){ const o=findOrder(target.dataset.order); if(o){o.status='processing';o.label='Poslato u Abacus';o.progress=2; closeModal();renderShell();toast(state.lang==='en'?'Order approved and sent to Abacus.':'Porudžbina je odobrena i poslata u Abacus.');} }
